...
export default function Admin() {
  ...
  const exportToExcel = () => {
    ...
    const wscols = [{wch: 10}, {wch: 30}, {wch: 40}, {wch: 20}, {wch: 20}, {wch: 20}, {wch: 25}];
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="flex bg-slate-50 border border-slate-200 rounded-lg p-1">
              <button 
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${statusFilter === 'Todos' ? 'bg-white shadow text-primary' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Todos
              </button>
              {STATUS_OPTIONS.map(opt => (
                <button 
                  ...
                  className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${statusFilter === opt.label ? 'bg-white shadow text-primary' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  {opt.label}
                </button>
              ))}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm shadow-sm focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
            />
          </div>
          <button 
            onClick={exportToExcel}
            disabled={leads.length === 0}
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg font-medium text-sm shadow-sm transition-colors disabled:opacity-50"
          >
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-slate-400">Carregando seus leads...</p>
                      </div>
                    </td>
                  </tr>
                  ...
                  filteredLeads.map((lead) => (
                    <tr key={lead.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="font-semibold text-slate-900 flex items-center gap-2 group-hover:text-primary transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5">
                          <select 
                            ...
                            className={`text-[11px] font-bold px-2.5 py-1.5 rounded-lg border-2 border-transparent focus:ring-2 focus:ring-primary shadow-sm outline-none transition-all cursor-pointer ${
                          ...
                          <div className="text-slate-400 text-[9px] ml-1 flex items-center gap-1">
